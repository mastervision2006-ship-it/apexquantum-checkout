...
/* ─────────────────────────────────────────────
   CUPOM — CRC16 e reescrita do EMV PIX
   ───────────────────────────────────────────── */
...
function buildDiscountedPixCode(originalCode, discountedAmount) {
  const amountStr = discountedAmount.toFixed(2);
  const amountField = `54${String(amountStr.length).padStart(2, "0")}${amountStr}`;
  // Slice off "6304XXXX" (8 chars), replace amount tag, re-append "6304", recompute CRC
  const body = originalCode.slice(0, -8).replace(/54\d{2}\d+\.\d{2}/, amountField) + "6304";
...
export default function CheckoutDesbloquear() {
  ...
  function getDiscountedPrice(plan) {
    ...
    const original = parseFloat(plan.price.replace(/\./g, "").replace(",", "."));
    const discounted = Math.max(original - appliedCoupon.discount, 1);
    return discounted.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  }

  function getPixCode(plan) {
    if (!appliedCoupon) return plan.pixCode;
    const original = parseFloat(plan.price.replace(/\./g, "").replace(",", "."));
    const discounted = Math.max(original - appliedCoupon.discount, 1);
    return buildDiscountedPixCode(plan.pixCode, discounted);
  }
  ...
  const handleCopyPix = () => {
    navigator.clipboard.writeText(getPixCode(currentPlan)).catch(() => {});
    setCopied(true);
  ...
  const displayPrice = appliedCoupon ? getDiscountedPrice(currentPlan) : currentPlan.price;
  const pixCode = getPixCode(currentPlan);
    ...
    <div style={{
      ...
      <div className="checkout-wrap" style={{ position: "relative", zIndex: 1, maxWidth: "1100px", margin: "0 auto", padding: "0 1.5rem" }}>
                ...
                <div style={{ marginBottom: "0.25rem", lineHeight: 1 }}>
                  ...
                  <div>
                    <span style={{
                      ...
                      color: hasDiscount ? "var(--gold)" : "var(--text-soft)",
                      verticalAlign: "top", lineHeight: "2.2",
                    }}>R$ </span>
                    <span style={{
                      fontFamily: "var(--font-price)",
                      fontSize: "clamp(2.2rem, 5vw, 2.8rem)", fontWeight: 800,
                      color: isSelected ? "var(--gold-bright)" : hasDiscount ? "var(--gold)" : "var(--text-base)",
                      transition: "color 0.3s ease",
                      letterSpacing: "-0.02em",
                    }}>
                      {hasDiscount ? discountedPrice : plan.price}
                    </span>
                  </div>
                </div>
                <div style={{
        ...
        <div style={{
          ...
          border: `1px solid ${appliedCoupon ? "rgba(34,197,94,0.35)" : "var(--border-gold)"}`,
          borderRadius: "20px",
          ...
          <p style={{
            ...
            Insira o código recebido do seu atendente para desbloquear os planos
          </p>
          <div style={{ display: "flex", gap: "10px" }}>
            <input
              type="text"
              placeholder="Ex: APEX100"
          ...
          {appliedCoupon && (
            <div style={{
              ...
              fontFamily: "var(--font-ui)", fontSize: "0.82rem",
              color: "#22c55e", marginTop: "0.75rem",
            }}>
              <GreenCheckIcon />
              Desconto de R${appliedCoupon.discount} aplicado com sucesso!
            </div>
          )}
          ...
          {couponError && (
            <div style={{
              fontFamily: "var(--font-ui)", fontSize: "0.82rem",
              color: "#F87171", marginTop: "0.75rem",
            }}>
              {couponError}
            </div>
          )}
            ...
            <>
              ...
              <div style={{
                ...
                <div style={{ textAlign: "right" }}>
                  {appliedCoupon && (
                    <div style={{
                      fontFamily: "var(--font-price)", fontSize: "0.95rem",
              ...
              <button
                ...
                style={{
                  marginTop: "0.5rem",
                  opacity: appliedCoupon ? 1 : 0.38,
                  cursor: appliedCoupon ? "pointer" : "not-allowed",
                  pointerEvents: appliedCoupon ? "auto" : "none",
                }}
              >
              ...
              {!appliedCoupon && (
                <p style={{
                  ...
                  <LockIcon /> Insira o código do seu atendente para continuar
                </p>
              )}
            ...
            <div style={{ textAlign: "center" }}>
              ...
              {appliedCoupon ? (
                <div style={{ marginBottom: "1.75rem" }}>
                  <div style={{
                    fontFamily: "var(--font-price)", fontSize: "1.3rem",
                    color: "var(--text-dim)", textDecoration: "line-through",
                  ...
                  <div style={{
                    ...
                    <GreenCheckIcon />
                    Código {appliedCoupon.code} — R${appliedCoupon.discount} de desconto
                  </div>
                </div>
              ) : (
                <div style={{
                  fontFamily: "var(--font-price)",
                  fontSize: "2.2rem", fontWeight: 800,
                  color: "var(--gold-bright)", marginBottom: "1.75rem",
                  letterSpacing: "-0.02em",
                }}>
                  R$ {currentPlan.price}
                </div>
              ...
              {/* QR Code — gerado com o valor correto (com desconto se aplicado) */}
              <div style={{
                display: "inline-flex",
                padding: "1.25rem",
                background: "#FFFFFF",
